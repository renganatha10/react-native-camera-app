...
async function executeWithDelimiter(command, delimiter) {
  await RNFFmpeg.execute(command, delimiter).then(result =>
    console.log('FFmpeg process exited with rc ' + result.rc)
  );
}
      ...
      <View style={commandScreenStyles.screenStyle}>
        ...
        <View style={commandScreenStyles.commandOutputViewStyle}>
          <ScrollView style={commandScreenStyles.commandOutputScrollViewStyle}>
            <Text style={commandScreenStyles.commandOutputTextStyle}>
  ...
  getMediaInformation = () => {
    RNFFmpeg.getMediaInformation(RNFS.CachesDirectoryPath + '/video.mp4').then(
      info => {
        ...
        this.setState({ path: `file:///${info.path}` });
